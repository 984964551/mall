...
import wac.mall.config.AlipayConfig;
...
import wac.mall.service.impl.OrderServiceImpl;
...
public class PayController {
    ...
    public void alipayReturnNotice(HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException{
        ...
        if(verify_result){//验证成功
            ...
            //2、要将产品的销售数量加1
            productService.updatenumber(orderService.findid(out_trade_no));
            //3.将支付后的商品从购物车中减少或移除
            cartService.updel(orderService.findbuyerid(out_trade_no),orderService.findid(out_trade_no) );
            //4.保存支付的时间
            orderService.savepaytime(new Date(),out_trade_no);
            out.println("验证成功");
        }else{
            //该页面可做页面美工编辑
